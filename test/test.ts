...
for (let i = 0; i < configs.length; ++ i) {
    ...
    const plot0: { xk: number[], yk: number[], title: string } = {xk: pts0.xk, yk: pts0.yk, title: 'REAL0'};
    let pts1: { xk: number[], yk: number[] } = ExpTools.getPoints(f1, config.XMIN, config.XMAX, config.N);
    const plot1: { xk: number[], yk: number[], title: string } = {xk: pts1.xk, yk: pts1.yk, title: 'REAL1'};
    ...
    const plot2: { xk: number[], yk: number[], title: string } = {xk: pts2.xk, yk: pts2.yk, title: 'APPROX'};
